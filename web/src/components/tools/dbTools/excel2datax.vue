<template>
  ...
  <div>
    ...
    <select-conf-common v-model="confTypeDataX" v-model:select-conf="selectConfDataX" />
    <select-conf-common v-model="confTypeHikapi" v-model:select-conf="selectConfHikapi" />
    <select-conf-common v-model="confTypeWriterJdbc" v-model:select-conf="selectConfWriterJdbc" />
  </div>
    ...
    @invoke-change-table="changeTable">
      <template #scripts="">
        ...
        <el-row>
          <el-col :span="6"> 选择DDLType </el-col>
          <el-col :span="8">
            <el-select
              v-model="ddlType"
              placeholder="请选择DDLType"
              size="large"
            >
              <el-option
                v-for="item in ddlTypes"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
        ...
        <el-row :gutter="30">
          <el-col :span="6"> 选择reader/writer </el-col>
          <el-col :span="8">
            <el-select
              v-model="readerType"
              placeholder="请选择reader"
              size="large"
            >
              <el-option
                v-for="item in dataxRaderTypes"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-col :span="8">
            <el-select
              v-model="writerType"
              placeholder="请选择writer"
              size="large"
            >
              <el-option
                v-for="item in dataxWriterTypes"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
        ...
        <div v-if="ifShowDatax">
          ...
          <el-button
            v-loading.fullscreen.lock="fullscreenLoading"
            @click="saveScript2Dolphin"
            >保存脚本到dolphinscheduler</el-button
          >
...
const confTypeDataX = ref("datax");
const selectConfDataX = ref("");
const confTypeHikapi = ref("hikapi");
const selectConfHikapi = ref("");
const confTypeWriterJdbc = ref("jdbc");
const selectConfWriterJdbc = ref("");
...
const ok = () => {
  var body = {
    ...
    writer_conf: selectConfWriterJdbc.value,
    ddlType: ddlType.value,
    parameter: selectConfDataX.value,
    confDolphin: confContentDolphin.value,
    hikapiConf: selectConfHikapi.value,
    excelInfo: {
