<template>
  <div class="p-4">
    <el-tabs v-model="activeTab" type="card">
      ...
      <el-tab-pane label="excel2json" name="excel2json">
        <el-card shadow="never" class="mb-4">
          <el-space wrap size="large">
            <select-conf-common
              v-model="confType"
              v-model:select-conf="selectConf"
            />
      ...
      <el-tab-pane label="excel2markdown" name="excel2markdown">
        <el-card shadow="never" class="mb-4">
          <el-space wrap size="large">
            <select-conf-common
              v-model="confType"
        ...
        <el-card shadow="never">
          <excel-info
            v-model="selectConf"
            v-model:ifExtractTable="ifExtractTable"
            @invoke-change-sheet="transform2markdown"
          >
            <template #scripts>
              <div v-if="ifShow" class="text-right mt-2">
                <el-button
                  type="primary"
                  @click="() => handleCopy(dataMarkDown)"
                  >复制结果</el-button
                >
              </div>
            </template>
...
const transform2markdown = (f, s) => {
  ifShow.value = false;
  DBToolsApi.process2markdown({
    filePath: f,
    sheet: s,
    filterExpr: pandasFilter.value,
    transformSteps: transformSteps.value,
